...
public class GameController implements Initializable, GameInterface {
	...
	/**
	 * 이전 스테이지, 다음 스테이지, 게임 시작, 나가기 이벤트 처리 버튼
	 */
...
//		mGameLevelNum = 1;
	...
	public void loadDataFile(int levelNum) {
		...
		initCanvas();
		// 레벨 별 stage title 갱신!
		((Stage)(lblTime.getScene().getWindow())).setTitle(MemberController.user.getMemberName()+"님 반갑습니다.- "+mGameLevelNum+" 레벨");	
		// 마우스 좌표값 초기화
		mouseX = mouseY = -1;
	}

	/**
	 * 3. 배열에 삽입된 정보로 canvas에 이미지 출력
	 */
	@Override
	...
	/**
	 * 클리어 시간 정보를 millisecond 에서 문자열로 변환
	 */
	...
	/**
	 * 나가기 또는 스테이지 닫기 버튼 선택 시 대기실로 이동
	 */
	public void showWaittingRoom() {
		try {
			...
			WaitingRoomController cont = loader.getController();
			cont.setStage(mGameLevelNum);
			stage.show();
